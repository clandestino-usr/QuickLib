...
router.get('/', (req, res) => {
  ...
  const total      = getCount(search, author, tags);
  const books      = getBooks({ search, author, tags, sort, limit: perPage, offset });
  const totalPages = Math.ceil(total / perPage);

  const viewOpts       = { sort, perPage, view };
  const content        = view === 'list' ? renderList(books, viewOpts) : renderGrid(books, viewOpts);
  const paginationArgs = { page, totalPages, perPage, sort, search, author, tags: tagsStr, view };
  ...
  res.send(renderPage({ content, topPagination, bottomPagination, search, author, tags: tagsStr, sort, perPage, view, total }));
});
